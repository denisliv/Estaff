...
├── utils/                  # Утилиты
│   ├── utils.py           # Вспомогательные функции
│   └── log_handler.py     # Обработчик логов для WebSocket
...
После запуска сервера веб-интерфейс доступен по адресу: http://localhost:8000

Интерфейс включает:
- **Форма поиска**: Ввод описания вакансии и фильтров (грейд, опыт)
- **Результаты поиска**: Отображение найденных кандидатов с оценками
- **Просмотр резюме**: Возможность просмотра HTML резюме каждого кандидата
- **Логи в реальном времени**: WebSocket подключение для отслеживания работы системы
...
Получение HTML резюме кандидата по имени и телефону.
...
WebSocket эндпоинт для получения логов системы в реальном времени.
